...
func RunGenerateArticle() {
	...
	generator := newArticleGenerator(client, s)
...
func (a *articleGenerator) generate(ctx context.Context, author *store.DigitalAuthorWithArticleSlugs) error {
	...
	err = a.store.CreateArticle(ctx, &store.Article{
		Slug:        output.Slug,
